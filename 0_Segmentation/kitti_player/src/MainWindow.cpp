...
void MainWindow::ros_init(ros::NodeHandle node, ros::NodeHandle private_nh)
{
    ...
    // private_nh.param("time_topic", str_time_topic_, std::string("/kitti/time"));
    ...
    // private_nh.param("time_pub", is_time_pub_, true);
...
//    cout << "left_color: " << is_left_color_image_pub_ << endl;
    ...
    // pc_pub_ = nh_.advertise<std_msgs::Int8>(str_time_topic_, 10);
...
void MainWindow::load_data()
{
    ...
    if(is_left_color_image_pub_) {
        ...
        publish_image(left_color_img_pub_, kitti_data_.left_color_image(), index_manager.index());
    }
    if(is_right_color_image_pub_) {
        kitti_data_.set_right_color_image(index_manager.index());
        publish_image(right_color_img_pub_, kitti_data_.right_color_image(), index_manager.index());
    }

    if(is_velodyne_pub_) {
        kitti_data_.set_velodyne(index_manager.index());
        // std::cout << kitti_data_.set_velodyne(index_manager.index()) << std::endl;

        publish_velodyne(pc_pub_, kitti_data_.velodyne_data());
    }

    // if(is_time_pub_) {
    //     kitti_data_.set_time(index_manager.index());
    //     publish_time(pc_pub_, time_data_.velodyne_data());
    // }
...
//    publish_image(color_label_pub_, kitti_data_.left_image(), str_color_label_topic_);
    ...
    // increase index
    index_manager.inc();
    std::cout << index_manager.index() << std::endl;
}
...
void MainWindow::publish_image(image_transport::Publisher& img_pub, cv::Mat& img, int frame_id)
{
  cv_bridge::CvImage cv_image;
  cv_image.header.seq = index_manager.index();
//    cv_image.header.stamp = ros::Time::now();
